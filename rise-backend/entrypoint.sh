...
if [ -z "${ADMIN_TOKEN}" ] || [ "${ADMIN_TOKEN}" = "null" ]; then
  ...
  # The script will exit here, but the pocketbase process will continue running.
  # This is not ideal, but it's a temporary debugging step.
  exit 1
fi
...
USERS_COLLECTION_RESPONSE=$(curl -s -H "Authorization: ${ADMIN_TOKEN}" "${POCKETBASE_URL}/api/collections")
USERS_COLLECTION_ID=$(echo "${USERS_COLLECTION_RESPONSE}" | jq -r '.items[] | select(.name == "users") | .id')

if [ -z "${USERS_COLLECTION_ID}" ] || [ "${USERS_COLLECTION_ID}" = "null" ]; then
...
echo "Initialization complete. Waiting for PocketBase server to exit."
wait